  ...
  export default function VerPedido() {
    ...
    const { mutate: eliminarDetalle } = useDeletePedido(); // endpoint para eliminar detalle
    ...
    const pedido: Pedido = pedidoData.data;
    const detalles = pedido.detalles ?? [];

    // Agrupamos los detalles después de tenerlos
    const detallesAgrupados = Object.values(
      detalles.reduce((acc, item) => {
        if (!acc[item.idCatalogo]) {
          acc[item.idCatalogo] = { ...item };
        } else {
          acc[item.idCatalogo].cantidad += item.cantidad;
          acc[item.idCatalogo].subtotal += item.subtotal;
        }
        return acc;
      }, {} as Record<number, DetallePedido>)
    );
          ...
          <Box display="flex" flexDirection="column" gap={2}>
            <Paper sx={{ p: 2 }}>
              ...
              {detallesAgrupados.length === 0 ? (
                ...
                detallesAgrupados.map((item) => (
                  <Paper key={item.idCatalogo} sx={{ p: 1.5, mb: 1 }}>
                    <Box display="flex" justifyContent="space-between" alignItems="center">
                      <Typography fontWeight="bold">{item.nombrePlato}</Typography>

                      {/* BOTÓN BORRAR POR ELEMENTO */}
 <Button
  size="small"
  color="error"
  variant="outlined"
  onClick={() =>
    eliminarDetalle(
      { idPedido: pedido.idPedido, iddetalle: item.idDetalle },
      { onSuccess: () => refetch() }
    )
  }
>
  Eliminar
                    ...
                    <Box display="flex" justifyContent="space-between">
                      <Typography variant="caption">Cantidad: x{item.cantidad}</Typography>
                      <Typography variant="caption">S/. {item.precioUnitario.toFixed(2)}</Typography>
                    </Box>
          ...
          <Box display="flex" flexDirection="column" gap={2}>
            <Paper sx={{ p: 2 }}>
              ...
              <Box mt={2}>
                ...
                <Typography fontWeight="bold">Total: S/. {total.toFixed(2)}</Typography>
              </Box>
              ...
              {mostrarPlatos &&
                platos.map((plato) => (
                  <Card key={plato.idCatalogo} sx={{ mb: 2 }}>
                    <CardContent>
                      <Typography fontWeight="bold">{plato.nombre}</Typography>
                      <Typography>S/. {plato.precio.toFixed(2)}</Typography>

                      <Typography color={plato.stock > 0 ? "green" : "error"}>
                        {plato.stock > 0 ? `Stock: ${plato.stock}` : "Agotado"}
                      </Typography>
                      ...
                      <Button
                        ...
                        onClick={() =>
                          agregarPlato(
                            {
                              idPedido: pedido.idPedido,
                              detalles: [{ idCatalogo: plato.idCatalogo, cantidad: 1 }],
                            },
                            { onSuccess: () => refetch() }
                          )
                        }
